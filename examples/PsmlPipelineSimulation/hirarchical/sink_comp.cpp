	...
	namespace psml
	{
        namespace Samples
        {
            ...
            sink_comp::sink_comp(const String &name)
            {
				...
				p1 = register_process(Wrapper_To_Call_P1, false);
				...
				p2 = register_process(Wrapper_To_Call_P2, false);
				p2->register_port(in2);
				p2->register_port(out2);
				//p1->register_port(g2); ///
				p2->register_sensitivity(in2);
				in2.bind(c, c_w); cout << "sink connector " << &c << " interface " <<  &c_w << endl;
				//g2.bind(c_fifo, fifo);

				p4 = register_process(Wrapper_To_Call_P4, false);
			...
			void sink_comp::P1(psml_process *ownerProcess)
			{
				...
				out1->put(_in1, signal_delay, ownerProcess);
			}
			//----------------------------------------------------
			void sink_comp::P2(psml_process *ownerProcess)
			{
				...
				out2->put(_in2, signal_delay, ownerProcess);
			}
			//----------------------------------------------------
