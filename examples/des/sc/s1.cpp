...
/*#include "/root/devel/systemc-2.3.1/src/sysc/kernel/sc_object_manager.h"
...
void s1::s1_box()
{
    //std::cout << "@time " <<  sc_time_stamp() << " in s1::s1_box " << stage1_input << std::endl;
    ...
    while(sop)
    {
        ...
        {
            // add sop to list l
            std::cout << sop->name() << std::endl;
            std::vector<sc_object *> children = sop->get_child_objects();
            for(int i = 0 ; i < children.size() ; i++)
            {
                if(strcmp(children[i]->kind(), "sc_module") == 0)
                    std::cout << "child " << children[i]->name() << std::endl;
            }
            std::cout << "---------------" << std::endl;
        }
